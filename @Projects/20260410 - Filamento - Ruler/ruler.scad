...
min_thickness = 0.1;   // Height at the outer edges
mark_depth = 0.4;
...
module cm_marks() {
    ...
    for (i = [0:length_cm]) {
        ...
        // Number every cm - centered on mark, use surface height at text position
        cm_text_y = width - 1.2;
        cm_text_z = surface_z(cm_text_y);
        // Center text: shift by half estimated text width
        cm_text_offset = font_size_mm / 3;
        translate([x - cm_text_offset, cm_text_y, cm_text_z - mark_depth])
...
module inch_marks() {
    ...
    for (i = [0:inches_mark_count]) {
        ...
        // Inch numbers - centered on mark, use surface height at text position
        inch_text_y = flip_inches ? 0.7 + font_size_in : 0.7;
        inch_text_z = surface_z(inch_text_y);
        // Center text: shift by half estimated text width (~font_size_in/3 for single digit)
        text_offset = font_size_in / 3;
        if (flip_inches) {
            translate([inch_pos + text_offset, inch_text_y, inch_text_z - mark_depth])
                rotate([0, 0, 180])
                linear_extrude(height = mark_depth + 0.01)
